...
static void apply_theme(uint32_t background, uint32_t foreground);
...
void VGA_tty_init(TTY_CONFIG *config)
{
    ...
    config->foreground_theme = 0x00ffffff;
    ...
    config->apply_theme = apply_theme;
    config->tty_ctx = NULL;
...
static void apply_theme(uint32_t background, uint32_t foreground)
{
    theme = color32_to_color8(foreground) | color32_to_color8(background) << 4;
}
